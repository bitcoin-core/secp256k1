...
int main(void) {
    ...
    unsigned char tx_outputs_ser[N_OUTPUTS][32];
    const unsigned char *tx_output_ser_ptrs[N_OUTPUTS];
    ...
    secp256k1_silentpayments_label_entry bob_label_entries[N_LABELS];
    const secp256k1_silentpayments_label_entry *bob_label_entries_ptrs[N_LABELS];
    ...
    for (i = 0; i < N_OUTPUTS; i++) {
        ...
        tx_output_ser_ptrs[i] = &tx_outputs_ser[i][0];
        found_output_ptrs[i] = &found_outputs[i];
    }
    for (i = 0; i < N_LABELS; i++) {
        bob_label_entries_ptrs[i] = &bob_label_entries[i];
    }
    ...
    /*** Create a labeled Silent Payments address (Bob) ***/
    {
        ...
        assert(ret);
        /* Create a (label_tweak, label) pair and add them to the label entries list.
         *
         * Bob MUST keep track of all of the labels he has used by adding them
         * to the label entries. Otherwise, he will not find outputs sent to his
         * labeled addresses when scanning.
         */
        ret = secp256k1_silentpayments_recipient_label_create(ctx,
            &bob_label_entries[0].label,
            bob_label_entries[0].label_tweak,
            bob_scan_key,
            m
        );
        if (!ret) {
        ...
        /* Now that the label entries have been updated, Bob creates his labeled
         * Silent Payments address and publishes it.
         */
        ret = secp256k1_silentpayments_recipient_create_labeled_spend_pubkey(ctx,
            &labeled_spend_pubkey, &unlabeled_spend_pubkey, &bob_label_entries[0].label);
        if (!ret) {
            printf("Something went wrong, event with negligible probability happened.\n");
            return EXIT_FAILURE;
        }
    ...
    /*** Sending (Alice) ***/
    {
        ...
        for (i = 0; i < N_OUTPUTS; i++) {
            ...
            assert(ret);
            print_hex(tx_outputs_ser[i], sizeof(tx_outputs_ser[i]));
        }
    ...
    {
        {
            ...
            ret = secp256k1_silentpayments_recipient_scan_outputs(ctx,
                ...
                tx_output_ser_ptrs, N_OUTPUTS,
                bob_scan_key,
                &prevouts_summary,
                &unlabeled_spend_pubkey,
                bob_label_entries_ptrs, N_LABELS /* NULL, 0 for no labels */
            );
            if (!ret) {
                printf("This transaction is not valid for Silent Payments, skipping.\n");
                return EXIT_SUCCESS;
            ...
            if (n_found_outputs > 0) {
                ...
                secp256k1_xonly_pubkey xonly_output_found, xonly_output_calculated;
                ...
                for (i = 0; i < n_found_outputs; i++) {
                    printf("    ");
                    print_hex(found_outputs[i].output, sizeof(found_outputs[i].output));
                    ret = secp256k1_xonly_pubkey_parse(ctx, &xonly_output_found, found_outputs[i].output);
                    assert(ret);
                     ...
                     */
                    assert(ret);
                    assert(secp256k1_xonly_pubkey_cmp(ctx, &xonly_output_calculated, &xonly_output_found) == 0);
