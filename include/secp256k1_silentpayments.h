 ...
 *           0 if the input private keys sum to zero, if the input private key
 *             sum is the negation of the spend secret key, if the input_hash
 *             or hash(shared_secret || k) are invalid scalars,
 ...
 *           0 if label_tweak32 is an invalid scalar,
 *             or if the arguments are invalid.
 *
 *  Args:                ctx: pointer to a context object
 *                            (not secp256k1_context_static)
 ...
 *           0 if the spend pubkey + label sum to zero,
 *             or if the arguments are invalid.
 *
 *  Args:                    ctx: pointer to a context object
 ...
 *             if the input_hash is an invalid scalar,
 ...
 *
 *  Given the public input data (secp256k1_silentpayments_prevouts_summary),
 ...
 *             or if the arguments are invalid.
 *  Args:                   ctx: pointer to a context object
 *  Out:           output_xonly: pointer to the resulting output x-only public key
 *  In:         shared_secret33: shared secret, derived from either sender's
 *                               or recipient's perspective with routines from
 *                               above
 *                 spend_pubkey: pointer to the recipient's spend public key
