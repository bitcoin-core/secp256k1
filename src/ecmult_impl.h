...
static int secp256k1_ecmult_multi_pippenger(secp256k1_gej *buckets, int bucketbits, struct secp256k1_ecmult_point_state_pippenger *state, secp256k1_gej *r, secp256k1_scalar *sc, secp256k1_ge *pt, size_t num) {
    ...
    for (np = 0; np < num; ++np) {
        ...
        state[no].skew_na = secp256k1_wnaf_fixed(state[no].wnaf_na, &sc[np], bucketbits+1);
        no++;
    }
    secp256k1_gej_set_infinity(r);
    ...
    for (i = WNAF_SIZE(bucketbits+1) - 1; i >= 0; i--) {
...
static int secp256k1_ecmult_multi_split_pippenger(const secp256k1_ecmult_context *ctx, secp256k1_scratch *scratch, const secp256k1_callback* error_callback, secp256k1_gej *r, const secp256k1_scalar *inp_g_sc, secp256k1_ecmult_multi_callback cb, void *cbdata, size_t n) {
    ...
    size_t entry_size = sizeof(secp256k1_ge) + sizeof(secp256k1_scalar) + sizeof(struct secp256k1_ecmult_point_state_pippenger) + (WNAF_SIZE(bucketbits+1))*sizeof(int);
    while (!secp256k1_scratch_resize(scratch, error_callback, (1<<bucketbits) * sizeof(secp256k1_gej) + (entries_per_batch + 2) * entry_size, 4 + (entries_per_batch + 2))) {
        ...
        entry_size = sizeof(secp256k1_ge) + sizeof(secp256k1_scalar) + sizeof(struct secp256k1_ecmult_point_state_pippenger) + (WNAF_SIZE(bucketbits+1))*sizeof(int);
        if (entries_per_batch < 2) {
            return 0;
        }
    }
    secp256k1_scratch_reset(scratch);
    pt = (secp256k1_ge *) secp256k1_scratch_alloc(scratch, (entries_per_batch + 2)* sizeof(*pt));
    sc = (secp256k1_scalar *) secp256k1_scratch_alloc(scratch, (entries_per_batch + 2) * sizeof(*sc));
    state_space = (struct secp256k1_ecmult_point_state_pippenger *) secp256k1_scratch_alloc(scratch, (entries_per_batch + 2) * sizeof(*state_space));
    VERIFY_CHECK(ctx != NULL);
    VERIFY_CHECK(pt != NULL);
    VERIFY_CHECK(sc != NULL);
    VERIFY_CHECK(state_space != NULL);
    for(i=0; i<entries_per_batch + 2; i++) {
        state_space[i].wnaf_na = (int *) secp256k1_scratch_alloc(scratch, (WNAF_SIZE(bucketbits+1)) * sizeof(int));
...
struct secp256k1_ecmult_point_state_pippenger_bin {
    size_t input_pos;
};

static int secp256k1_ecmult_multi_pippenger_bin(secp256k1_gej *buckets, int bucketbits, struct secp256k1_ecmult_point_state_pippenger_bin *state, secp256k1_gej *r, secp256k1_scalar *sc, secp256k1_ge *pt, size_t num) {
...
static int secp256k1_ecmult_multi_pippenger_bin_bucketbits(size_t n) {
#ifdef USE_ENDOMORPHISM
    if (n < 3) {
        return 1;
    } else if (n < 8) {
...
static int secp256k1_ecmult_multi_split_pippenger_bin(const secp256k1_ecmult_context *ctx, secp256k1_scratch *scratch, const secp256k1_callback* error_callback, secp256k1_gej *r, const secp256k1_scalar *inp_g_sc, secp256k1_ecmult_multi_callback cb, void *cbdata, size_t n) {
    ...
    secp256k1_scalar *sc;
    secp256k1_gej *buckets;
    struct secp256k1_ecmult_point_state_pippenger_bin *state_space;
    size_t idx = 0;
    size_t point_idx = 0;

    /* Attempt to allocate sufficient space for Bos-Coster */
    int bucketbits = secp256k1_ecmult_multi_pippenger_bin_bucketbits(entries_per_batch);
    while (!secp256k1_scratch_resize(scratch, error_callback, (1<<bucketbits) * sizeof(secp256k1_gej) + entries_per_batch * entry_size, 4)) {
        entries_per_batch /= 2;
        bucketbits = secp256k1_ecmult_multi_pippenger_bin_bucketbits(entries_per_batch);
        if (entries_per_batch < 2) {
            return 0;
        }
    }
    ...
    state_space = (struct secp256k1_ecmult_point_state_pippenger_bin *) secp256k1_scratch_alloc(scratch, entries_per_batch * sizeof(*state_space));
    buckets = (secp256k1_gej *) secp256k1_scratch_alloc(scratch, (1<<bucketbits) * sizeof(*buckets));
    ...
    while (point_idx < n) {
        ...
        if (idx >= entries_per_batch) {
#endif
            secp256k1_ecmult_multi_pippenger_bin(buckets, bucketbits, state_space, &tmp, sc, pt, idx);
            secp256k1_gej_add_var(r, r, &tmp, NULL);
            idx = 0;
        }
        point_idx++;
    }
    secp256k1_ecmult_multi_pippenger_bin(buckets, secp256k1_ecmult_multi_pippenger_bin_bucketbits(idx), state_space, &tmp, sc, pt, idx);
    secp256k1_gej_add_var(r, r, &tmp, NULL);
    return 1;
}
...
static int secp256k1_ecmult_multi(const secp256k1_ecmult_context *ctx, secp256k1_scratch *scratch, const secp256k1_callback* error_callback, secp256k1_gej *r, const secp256k1_scalar *inp_g_sc, secp256k1_ecmult_multi_callback cb, void *cbdata, size_t n) {
    ...
    /*return secp256k1_ecmult_multi_split_pippenger_bin(ctx, scratch, error_callback, r, inp_g_sc, cb, cbdata, n);*/
}
