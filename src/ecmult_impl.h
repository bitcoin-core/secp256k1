...
static int secp256k1_ecmult_multi(
    ...
    size_t max_batch_size = secp256k1_ecmult_multi_batch_size(mem_limit);
    size_t offset = 0;
    secp256k1_gej tmp;
    secp256k1_ecmult_multi_algo algo;
    size_t chunk;

    /* If all points fit in memory, process in one shot. */
    if (n_points <= max_batch_size) {
        algo = secp256k1_ecmult_multi_select(mem_limit, n_points);
        return secp256k1_ecmult_multi_internal(error_callback, algo, r, n_points, points, scalars, scalar_g);
    }

    /* Otherwise, split into batches based on mem_limit. */
    secp256k1_gej_set_infinity(r);

    while (offset < n_points) {
        chunk = n_points - offset;
        if (chunk > max_batch_size) {
            chunk = max_batch_size;
        }

        algo = secp256k1_ecmult_multi_select(mem_limit, chunk);
        /* scalar_g is only included in the first batch */
        if (!secp256k1_ecmult_multi_internal(error_callback, algo, &tmp, chunk,
                                             points + offset, scalars + offset,
                                             offset == 0 ? scalar_g : NULL)) {
            return 0;
        }
