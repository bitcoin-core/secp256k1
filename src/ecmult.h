 ...
 *
 * Note: The algorithm chosen here may differ from what
 * secp256k1_ecmult_multi_select returns for the same mem_limit and
 * batch_size returned for this mem_limit here. This can happend
 * because _select evaluates all algorithms at the given batch_size
 * only.
 *
 * Returns: The optimal batch size. Falls back to ECMULT_MAX_POINTS_PER_BATCH
 ...
 *
 * Chooses the best algorithm for the given number of points and
 * mem_limit. If mem_limit is too small to process all points at once,
 * splits into batches and accumulates results. With mem_limit == 0 the
 * trivial algorithm is used, which does not allocate any memory.
 *
