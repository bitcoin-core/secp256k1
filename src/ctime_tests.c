...
static void run_tests(secp256k1_context *ctx, unsigned char *key) {
    ...
    unsigned char tx_output[32];
    const unsigned char *tx_outputs[1];
    ...
    CHECK(secp256k1_xonly_pubkey_serialize(ctx, tx_output, generated_outputs[0]));
    tx_outputs[0] = &tx_output[0];
    found_outputs_ptrs[0] = &found_outputs[0];
    n_found_outputs = 1;
    SECP256K1_CHECKMEM_DEFINE(&recipient.spend_pubkey, sizeof(recipient.spend_pubkey));
    /* It is sufficient to check _recipient_scan_outputs without passing labels, since the shared secret is created once (which is where the constant timeness matters)
     * and then reused for the rest of the scanning logic.
     */
    CHECK(secp256k1_silentpayments_recipient_scan_outputs(ctx, found_outputs_ptrs, &n_found_outputs, tx_outputs, 1, key, &prevouts_summary, &recipient.spend_pubkey, NULL, 0));
