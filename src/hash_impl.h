...
static void secp256k1_hash_context_init(secp256k1_hash_context* hash_ctx) {
    ...
    hash_ctx->fn_sha256_transform = secp256k1_sha256_transform;
}
...
static void secp256k1_sha256_write(secp256k1_sha256 *hash, const unsigned char *data, size_t len, const secp256k1_hash_context *ctx) {
    ...
    VERIFY_CHECK(ctx->fn_sha256_transform != NULL);
    while (len >= 64 - bufsize) {
        ...
        data += chunk_len;
        len -= chunk_len;
        ctx->fn_sha256_transform(hash->s, hash->buf, 1);
        bufsize = 0;
    }
    if (len) {
...
static void secp256k1_sha256_finalize(secp256k1_sha256 *hash, unsigned char *out32, const secp256k1_hash_context *ctx) {
    ...
    secp256k1_sha256_write(hash, pad, 1 + ((119 - (hash->bytes % 64)) % 64), ctx);
    secp256k1_sha256_write(hash, sizedesc, 8, ctx);
    for (i = 0; i < 8; i++) {
...
static void secp256k1_sha256_initialize_tagged(secp256k1_sha256 *hash, const unsigned char *tag, size_t taglen, const secp256k1_hash_context *ctx) {
    unsigned char buf[32];
    secp256k1_sha256_initialize(hash);
    secp256k1_sha256_write(hash, tag, taglen, ctx);
    secp256k1_sha256_finalize(hash, buf, ctx);

    secp256k1_sha256_initialize(hash);
    secp256k1_sha256_write(hash, buf, 32, ctx);
    secp256k1_sha256_write(hash, buf, 32, ctx);
}
...
static void secp256k1_hmac_sha256_initialize(secp256k1_hmac_sha256 *hash, const unsigned char *key, size_t keylen, const secp256k1_hash_context *ctx) {
    size_t n;
    ...
    if (keylen <= sizeof(rkey)) {
        ...
        secp256k1_sha256_write(&sha256, key, keylen, ctx);
        secp256k1_sha256_finalize(&sha256, rkey, ctx);
        memset(rkey + 32, 0, 32);
    }
    ...
    secp256k1_sha256_write(&hash->outer, rkey, sizeof(rkey), ctx);

    secp256k1_sha256_initialize(&hash->inner);
    for (n = 0; n < sizeof(rkey); n++) {
        rkey[n] ^= 0x5c ^ 0x36;
    }
    secp256k1_sha256_write(&hash->inner, rkey, sizeof(rkey), ctx);
...
static void secp256k1_hmac_sha256_write(secp256k1_hmac_sha256 *hash, const unsigned char *data, size_t size, const secp256k1_hash_context *ctx) {
    secp256k1_sha256_write(&hash->inner, data, size, ctx);
}
...
static void secp256k1_hmac_sha256_finalize(secp256k1_hmac_sha256 *hash, unsigned char *out32, const secp256k1_hash_context *ctx) {
    ...
    secp256k1_sha256_finalize(&hash->inner, temp, ctx);
    secp256k1_sha256_write(&hash->outer, temp, 32, ctx);
    secp256k1_memclear_explicit(temp, sizeof(temp));
    secp256k1_sha256_finalize(&hash->outer, out32, ctx);
}
...
static void secp256k1_rfc6979_hmac_sha256_initialize(secp256k1_rfc6979_hmac_sha256 *rng, const unsigned char *key, size_t keylen, const secp256k1_hash_context *ctx) {
    ...
    /* RFC6979 3.2.d. */
    secp256k1_hmac_sha256_initialize(&hmac, rng->k, 32, ctx);
    secp256k1_hmac_sha256_write(&hmac, rng->v, 32, ctx);
    secp256k1_hmac_sha256_write(&hmac, zero, 1, ctx);
    secp256k1_hmac_sha256_write(&hmac, key, keylen, ctx);
    secp256k1_hmac_sha256_finalize(&hmac, rng->k, ctx);
    secp256k1_hmac_sha256_initialize(&hmac, rng->k, 32, ctx);
    secp256k1_hmac_sha256_write(&hmac, rng->v, 32, ctx);
    secp256k1_hmac_sha256_finalize(&hmac, rng->v, ctx);

    /* RFC6979 3.2.f. */
    secp256k1_hmac_sha256_initialize(&hmac, rng->k, 32, ctx);
    secp256k1_hmac_sha256_write(&hmac, rng->v, 32,ctx);
    secp256k1_hmac_sha256_write(&hmac, one, 1, ctx);
    secp256k1_hmac_sha256_write(&hmac, key, keylen, ctx);
    secp256k1_hmac_sha256_finalize(&hmac, rng->k, ctx);
    secp256k1_hmac_sha256_initialize(&hmac, rng->k, 32, ctx);
    secp256k1_hmac_sha256_write(&hmac, rng->v, 32, ctx);
