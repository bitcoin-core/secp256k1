...
static int secp256k1_silentpayments_create_output_tweak(const secp256k1_hash_ctx *hash_ctx, secp256k1_scalar *t_k_scalar, const unsigned char *shared_secret33, uint32_t k) {
    secp256k1_sha256 hash;
...
static int secp256k1_silentpayments_create_output_pubkey(const secp256k1_context *ctx, secp256k1_xonly_pubkey *output_xonly, const unsigned char *shared_secret33, const secp256k1_pubkey *spend_pubkey, uint32_t k) {
     ...
     */
    if (!secp256k1_silentpayments_create_output_tweak(secp256k1_get_hash_context(ctx), &t_k_scalar, shared_secret33, k)) {
        return 0;
    }
     ...
     * to protect against this function being called with a malicious inputs, i.e.,
...
int secp256k1_silentpayments_recipient_scan_outputs(
    ...
    for (k = 0; k < k_max; k++) {
         ...
         */
        if (!secp256k1_silentpayments_create_output_tweak(secp256k1_get_hash_context(ctx), &t_k_scalar, shared_secret, k)) {
            secp256k1_scalar_clear(&t_k_scalar);
            secp256k1_memclear_explicit(&shared_secret, sizeof(shared_secret));
            return 0;
        }
