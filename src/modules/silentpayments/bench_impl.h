...
static void bench_silentpayments_output_scan(void* arg, int iters) {
    ...
    const secp256k1_pubkey *spend_ptrs[1];
    spend_ptrs[0] = &data->spend_pubkeys[0];

    for (i = 0; i < iters; i++) {
        secp256k1_xonly_pubkey *xonly_output_ptrs[1];
        secp256k1_xonly_pubkey xonly_outputs[1];
        xonly_output_ptrs[0] = &xonly_outputs[0];
        CHECK(secp256k1_silentpayments_recipient_prevouts_summary_parse(data->ctx, &prevouts_summary, data->input_pubkey33, 33));
