...
static int nonce_function_bip340_impl(const secp256k1_hash_context *hash_ctx, unsigned char *nonce32, const unsigned char *msg, size_t msglen, const unsigned char *key32, const unsigned char *xonly_pk32, const unsigned char *algo, size_t algolen, void *data) {
    ...
    if (data != NULL) {
        ...
        secp256k1_sha256_write(&sha, data, 32, hash_ctx);
        secp256k1_sha256_finalize(&sha, masked_key, hash_ctx);
        for (i = 0; i < 32; i++) {
    ...
    if (algolen == sizeof(bip340_algo)
        ...
        secp256k1_sha256_initialize_tagged(&sha, algo, algolen, hash_ctx);
    }

    /* Hash masked-key||pk||msg using the tagged hash as per the spec */
    secp256k1_sha256_write(&sha, masked_key, 32, hash_ctx);
    secp256k1_sha256_write(&sha, xonly_pk32, 32, hash_ctx);
    secp256k1_sha256_write(&sha, msg, msglen, hash_ctx);
    secp256k1_sha256_finalize(&sha, nonce32, hash_ctx);
