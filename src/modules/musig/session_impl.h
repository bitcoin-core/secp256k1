...
static void secp256k1_nonce_function_musig_helper(const secp256k1_context *ctx, secp256k1_sha256 *sha, unsigned int prefix_size, const unsigned char *data, unsigned char len) {
    ...
    secp256k1_sha256_write(sha, zero, prefix_size - 1, secp256k1_get_hash_context(ctx));
    if (data != NULL) {
        secp256k1_sha256_write(sha, &len, 1, secp256k1_get_hash_context(ctx));
        secp256k1_sha256_write(sha, data, len, secp256k1_get_hash_context(ctx));
    } else {
        len = 0;
        secp256k1_sha256_write(sha, &len, 1, secp256k1_get_hash_context(ctx));
    }
...
static void secp256k1_nonce_function_musig(const secp256k1_context *ctx, secp256k1_scalar *k, const unsigned char *session_secrand, const unsigned char *msg32, const unsigned char *seckey32, const unsigned char *pk33, const unsigned char *agg_pk32, const unsigned char *extra_input32) {
    ...
    if (seckey32 != NULL) {
        ...
        secp256k1_sha256_write(&sha, session_secrand, 32, secp256k1_get_hash_context(ctx));
        secp256k1_sha256_finalize(&sha, rand, secp256k1_get_hash_context(ctx));
        for (i = 0; i < 32; i++) {
            rand[i] ^= seckey32[i];
        }
    } else {
    ...
    secp256k1_sha256_write(&sha, rand, sizeof(rand), secp256k1_get_hash_context(ctx));
    secp256k1_nonce_function_musig_helper(ctx, &sha, 1, pk33, 33);
    secp256k1_nonce_function_musig_helper(ctx, &sha, 1, agg_pk32, 32);
    msg_present = msg32 != NULL;
    secp256k1_sha256_write(&sha, &msg_present, 1, secp256k1_get_hash_context(ctx));
    if (msg_present) {
        secp256k1_nonce_function_musig_helper(ctx, &sha, 8, msg32, 32);
    }
    secp256k1_nonce_function_musig_helper(ctx, &sha, 4, extra_input32, 32);

    for (i = 0; i < 2; i++) {
        unsigned char buf[32];
        secp256k1_sha256 sha_tmp = sha;
        secp256k1_sha256_write(&sha_tmp, &i, 1, secp256k1_get_hash_context(ctx));
        secp256k1_sha256_finalize(&sha_tmp, buf, secp256k1_get_hash_context(ctx));
...
static int secp256k1_musig_nonce_gen_internal(const secp256k1_context* ctx, secp256k1_musig_secnonce *secnonce, secp256k1_musig_pubnonce *pubnonce, const unsigned char *input_nonce, const unsigned char *seckey, const secp256k1_pubkey *pubkey, const unsigned char *msg32, const secp256k1_musig_keyagg_cache *keyagg_cache, const unsigned char *extra_input32) {
    ...
    secp256k1_nonce_function_musig(ctx, k, input_nonce, msg32, seckey, pk_ser, aggpk_ser_ptr, extra_input32);
...
static void secp256k1_musig_compute_noncehash(const secp256k1_context *ctx, unsigned char *noncehash, secp256k1_ge *aggnonce, const unsigned char *agg_pk32, const unsigned char *msg) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        secp256k1_sha256_write(&sha, buf, sizeof(buf), secp256k1_get_hash_context(ctx));
    }
    secp256k1_sha256_write(&sha, agg_pk32, 32, secp256k1_get_hash_context(ctx));
    secp256k1_sha256_write(&sha, msg, 32, secp256k1_get_hash_context(ctx));
    secp256k1_sha256_finalize(&sha, noncehash, secp256k1_get_hash_context(ctx));
}
...
static void secp256k1_musig_nonce_process_internal(const secp256k1_context *ctx, int *fin_nonce_parity, unsigned char *fin_nonce, secp256k1_scalar *b, secp256k1_ge *aggnonce_pts, const unsigned char *agg_pk32, const unsigned char *msg) {
    ...
    secp256k1_musig_compute_noncehash(ctx, noncehash, aggnonce_pts, agg_pk32, msg);
...
int secp256k1_musig_nonce_process(const secp256k1_context* ctx, secp256k1_musig_session *session, const secp256k1_musig_aggnonce  *aggnonce, const unsigned char *msg32, const secp256k1_musig_keyagg_cache *keyagg_cache) {
    ...
    secp256k1_schnorrsig_challenge(ctx, &session_i.challenge, fin_nonce, msg32, 32, agg_pk32);
...
int secp256k1_musig_partial_sign(const secp256k1_context* ctx, secp256k1_musig_partial_sig *partial_sig, secp256k1_musig_secnonce *secnonce, const secp256k1_keypair *keypair, const secp256k1_musig_keyagg_cache *keyagg_cache, const secp256k1_musig_session *session) {
    ...
    secp256k1_musig_keyaggcoef(ctx, &mu, &cache_i, &pk);
...
int secp256k1_musig_partial_sig_verify(const secp256k1_context* ctx, const secp256k1_musig_partial_sig *partial_sig, const secp256k1_musig_pubnonce *pubnonce, const secp256k1_pubkey *pubkey, const secp256k1_musig_keyagg_cache *keyagg_cache, const secp256k1_musig_session *session) {
     ...
     * much easier to do. */
    secp256k1_musig_keyaggcoef(ctx, &mu, &cache_i, &pkp);
