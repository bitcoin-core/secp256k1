...
static void secp256k1_num_mul_word_shift(secp256k1_num *r, const secp256k1_num *a, secp256k1_num_word w, int shift_w) {
    int i;
...
SECP256K1_INLINE static void secp256k1_num_leading_digit(secp256k1_num_word *rdigit, int *rindex, const secp256k1_num *a) {
    *rindex = NUM_N_WORDS;
    *rdigit = 0;
    while (*rindex > 0 && *rdigit == 0) {
        --*rindex;
    ...
    *rr = *a;
    ...
    for (i = NUM_N_WORDS - 2; i >= 0; --i) {
        ...
        if (rq->data[i] > 0) {
             ...
             * `secp256k1_num_mul_word_shift` to this. */
 ...
 * Returns the index of the highest nonzero digit of the quotient (or 0 if the quotient is 0) */
static int secp256k1_num_div_mod(secp256k1_num *rq, secp256k1_num *rr, const secp256k1_num *a, const secp256k1_num *m) {
    ...
    /* 0 / anything is equal to 0 remainder 0 */
    if (secp256k1_num_is_zero(a)) {
        memmove(rq, a, sizeof *a);
        memmove(rr, a, sizeof *a);
        return 0;
    }
     ...
     * function with a divisor which is <= 2^256. */
{
    ...
    while (output_idx > 0) {
        ...
        if (q > 0) {
            ...
            secp256k1_num_mul_word_shift(&sub, &div, q, output_idx - 1);
